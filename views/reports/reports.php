...
foreach($report as $r) {
    if($r['type'] == 'Income') {
        ...
        $total_expense += (hasRole('Admin') ? floatval($r['admin_cost']) : floatval($r['amount']));
    }
}
?>
                ...
                <tbody>
                    ...
                    <?php else: foreach($report as $r): ?>
                        <tr>
                            ...
                            <td><?= $r['description'] ?></td>
                            <td class="text-end pe-3 fw-bold">৳<?= number_format(($r['type'] == 'Expense' && hasRole('Admin')) ? $r['admin_cost'] : $r['amount'], 2) ?></td>
                        </tr>
