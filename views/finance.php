...
$tx_query = "SELECT * FROM ".TBL_FIN_CASHBOOK." WHERE 1=1";
$tx_params = [];
...
// No LIMIT when a date range is active – return all rows in that range.
if (isset($_GET['from_date']) || isset($_GET['to_date'])) {
    $tx_query .= " ORDER BY id DESC";
} else {
    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white py-3">
                <h6 class="mb-0 fw-bold text-dark"><i class="fas fa-book me-2"></i> Daily Cash Book (Master Ledger)</h6>
            </div>
                            ...
                            <?php foreach($transactions as $tx): ?>
                                <tr>
                                    ...
                                    <td class="text-end fw-bold <?= $tx['entry_type'] == 'Income' ? 'text-success' : 'text-danger' ?>">
                                        <?= $tx['entry_type'] == 'Income' ? '+' : '-' ?>৳<?= number_format(abs($tx['amount']), 2) ?>
                                    </td>
                                    <td class="text-end pe-3 fw-bold">৳<?= number_format($tx['running_balance'], 2) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
