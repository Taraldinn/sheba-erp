...
$tj_boxes = safeFetchAll($pdo, "SELECT t.*, z.name as zone_name FROM ".TBL_TJ_BOXES." t LEFT JOIN ".TBL_ZONES." z ON t.zone_id = z.id WHERE t.staff_id=? ORDER BY t.id DESC", [$owner_id]);
...
function parseFiberCodes(fiberCodeStr) {
    ...
    try {
        ...
        list = fiberCodeStr.split('\n').map(line => line.trim()).filter(line => line).map(line => {
            let code = line;
            let brand = '';
            // If the code starts with a color or brand pattern (legacy)
