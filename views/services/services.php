                    ...
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                                ...
                                <li>
                                    <button type="button"
                                        class="dropdown-item btn-sync-service"
                                        data-service="<?= $svcJson ?>">
                                        <i class="fas fa-sync me-2 text-success"></i>Sync Clients
                                    </button>
                                </li>
            ...
            <div class="modal-body">
                ...
                <div class="row g-2 mb-3">
                    ...
                    <div class="col-4">
                        ...
                        <input type="number" step="0.01" name="vat_percent" id="svc_vat" class="form-control" placeholder="5" value="0">
                    </div>
                </div>
                <div class="mb-3">
                     <label class="form-label fw-bold text-success">Final Sale Amount (User Pays)</label>
                     <input type="text" id="svc_total" class="form-control fw-bold text-success" readonly value="0.00">
                     <div class="form-text small">Selling Price + VAT</div>
                </div>
                <div class="mb-3">
    ...
    <div class="modal-dialog">
        <form method="POST" class="modal-content">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title"><i class="fas fa-sync me-2"></i>Sync Clients from MikroTik</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <input type="hidden" name="pkg_id" id="sync_pkg_id">
                <input type="hidden" name="pkg_name" id="sync_pkg_name">
                <input type="hidden" name="profile_name" id="sync_profile_name">
                
                <div class="alert alert-info py-2 small">
                    <i class="fas fa-info-circle me-1"></i> This will import all PPP secrets from the selected router that match this package's profile.
                </div>
                
                <div class="mb-3">
                    <label class="form-label fw-bold">Package:</label>
                    <div id="sync_pkg_display" class="form-control-plaintext fw-bold text-primary"></div>
                </div>
                
                <div class="mb-3">
                    <label class="form-label fw-bold small">Select MikroTik Router</label>
                ...
                <div class="mb-3">
                    <label class="form-label fw-bold small">Select Reseller</label>
                    <select name="reseller_id" class="form-select select2-reseller" style="width: 100%;" required>
                        <option value="">-- Choose Reseller --</option>
                        <option value="<?= $_SESSION['admin_id'] ?>">Self (<?= $_SESSION['admin_username'] ?>)</option>
                        <?php 
                        $resellers = safeFetchAll($pdo, "SELECT id, name FROM ".TBL_STAFF." WHERE role = 'Reseller'");
                ...
                <div class="form-check mb-3">
                    <input class="form-check-input" type="checkbox" name="auto_active" value="1" id="autoActive" checked>
                    <label class="form-check-label small" for="autoActive">
                        Automatically set imported clients as 'Active'
                    </label>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                <button type="submit" name="sync_mikrotik_clients" class="btn btn-success px-4">Start Import</button>
            </div>
        </form>
    </div>
</div>
