    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_olt':
                ...
                if ($result) $message = "OLT added successfully";
            ...
            case 'edit_olt':
                ...
                if ($result) $message = "OLT updated successfully";
                else $error = "Failed to update OLT";
                break;
            case 'delete_olt':
                if ($olt_manager->remove_olt($_POST['ip'])) $message = "OLT removed successfully";
...
// Determine if we need to trigger sync on load (cache older than 30s)
$needs_sync = (time() - filemtime($cache_file)) >= $cache_time || count($monitoring_data['onus']) == 0;
